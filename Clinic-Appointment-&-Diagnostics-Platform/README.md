...
> Note: In the image of the designed DB, if the attributes & their details aren't visible clearly then please open it in a new tab by right clicking on it. Or you can do just clone it an open in VS code for further clear image.

## Entities & Attributes

| Entity | Primary Key | Key Attributes | Description |
|---|---|---|---|
| `Patient` | `patient_id` | `first_name`, `last_name`, `date_of_birth`, `gender`, `phone_number`, `email`, `address`, `created_at`, `updated_at` | Stores patient profile and contact information. This is the base entity for appointments, consultations, and payments. |
...
| `Test` | `test_id` | `consultation_id` (FK), `test_name`, `description`, `prescribed_date`, `status`, `fee`, `created_at`, `updated_at` | Stores diagnostic tests prescribed during consultation. Each test has its own status and fee for billing and lab tracking. |
...
| `Payment` | `payment_id` | `consultation_id` (FK), `amount`, `payment_date`, `payment_method`, `status`, `notes`, `created_at` | Stores consultation payment transaction details. Amount is intended to include consultation fee plus all prescribed test fees for that consultation. |
